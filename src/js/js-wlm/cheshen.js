$(function() {
    ...
    for (var i = 0; i < li.length; i++) { //缩略图点击事件换相应的中图和大图
        ...
        $(li[i]).on('click', function() {
            ...
            obj.middleImg.attr("src", "../images/image-wlm/" + ($(li).index(this) + 108) + ".jpg");
            obj.bigImg.attr('src', "../images/image-wlm/" + ($(li).index(this) + 114) + ".jpg")
    ...
    $(".jcarousel-next").on("click", function(ev) {
        ...
        if (li.length > s + 1) {
            ...
            obj.middleImg.attr("src", "../images/image-wlm/" + (s + 108) + ".jpg");
        }
